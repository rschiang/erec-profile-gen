...
$(window).konami({
	...
	cheat: function() {
		...
		$('.preview').animate({'top':'-500px', 'opacity':'.5'}, 3000).animate({'width':'0', 'opacity':'0'}, 3000, function(){
			$('#content').slideUp();
		})
	}
});
...
function resizeDragger(size, wrapper, value, upload)
{
	...
	if(size[0] > size[1]) {
		...
		height = wrapper;
		top = 0;
		left = (width - wrapper) * .5 * -1;
	}
	else {
		scale = (wrapper / size[0]);
		width = wrapper;
		height = size[1] * scale;
		top = (height - wrapper) * .5 * -1;
		left = 0;
	}

	if(value == 6){
		left = wrapper * .2 * -1;
		if(size[0] > size[1]) left -= (width - wrapper) * .5;
	}
	else if(value == 9){
		$sizer.slider('value', 65);
		if(size[0] > size[1]) {
			left = wrapper * .65 * .13 * .5;
			width *= .65;
			height *= .65;
			top = (wrapper - height) * .48;
		}
		else {
			left = wrapper * .65 * .13 * .5;
			width *= .65;
...
function getBackgroundCenterPoint(size, position){
	return [size[0] * .5 + position[0], size[1] * .5 + position[1]]
}
